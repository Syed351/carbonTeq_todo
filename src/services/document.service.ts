...
export class DocumentService {
  ...
  async deleteDocument(documentId: string, userId: string, role: string): Promise<Result<void, string>> {
    this.logger.info("Deleting document", { documentId, userId, role });
    ...
    return Result.Ok(documentId)
      ...
      .map(() => {
        this.logger.info("Document deleted", { documentId, userId });
      })
      .mapErr((err) => {
  ...
  async updateDocument(
    documentId: string,
    data: Partial<IDocumentDTO>,
    file?: Express.Multer.File
  ): Promise<Result<void, string>> {
    return Result.Ok(documentId)
      .flatMap((id) => this.documentRepo.findById(id))
      .flatMap((document) => {
        const updatedData: IDocumentUpdateDTO = {
          id: documentId,
  ...
  async generateDownloadLink(
    ...
    return Result.Ok(documentId)
      ...
      .mapErr((err) => {
        this.logger.error("Failed to generate download link", { documentId, userId, error: err });
        return err;
      })
      .toPromise();
  }
  ...
  async downloadDocument(
    ...
    return Result.Ok(token)
      ...
      .flatMap((decoded) =>
        typeof decoded === "object" && decoded !== null && "documentId" in decoded
  ...
  async searchDocuments(tags: string | undefined): Promise<Result<IDocumentDTO[], string>> {
    ...
    return Result.Ok(tags)
      ...
      .flatMap(async (tagArray) => 
        tagArray.length > 0 
          ? this.documentRepo.searchByTags(tagArray)
          : Result.Ok([])
      )
      .map((docs) => {
